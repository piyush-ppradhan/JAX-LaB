...
class PoolFluid(MultiphaseMRT):
    ...
    def set_boundary_conditions(self):
        ...
        for i in range(self.n_components):
            self.BCs[i].append(BounceBack(bottom, self.gridInfo, self.precisionPolicy, theta=contact_angle))
            self.BCs[i].append(BounceBack(top, self.gridInfo, self.precisionPolicy))
    ...
    def compute_buoyancy_force(self, rho_tree, timestep):
        ...
        gravity_vector = jnp.array([0.0, -gravity], dtype=self.precisionPolicy.compute_dtype)
        gravity_active = jnp.asarray(timestep > gravity_relaxation_steps, dtype=self.precisionPolicy.compute_dtype)
